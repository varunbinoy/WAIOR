...
def main():
    ...
    # Dynamic faculty conflict (DTI split)
    for sl in slot_ids:
    ...
    # Cap per section and objective maximize total
    sec_sessions = {}
    for sec in section_ids:
    ...
    print("\nSolving (maximize total, cap=20, DTI faculty split ON)...")
    ...
    print("Max total sessions:", solver.Value(total_sessions), f"/ {len(section_ids)*CAP}")
    ...
    out = pd.DataFrame(rows).sort_values(["sessions","course_id","section_id"])
    out_path = OUTPUT_DIR / "section_sessions_max_cap20_facsplit.csv"
